...
public class PlayerLetterColumn4 extends Box {
    private Color color;
    ...
    public PlayerLetterColumn4(Color color, Dimension size) {
        this.color = color;
        bounds = new Rectangle(new Point(275, 0), size); // this is being used for calculating where the letter is
    ...
    private void resetSquare() {
        randomDelta = (int) (Math.random() * 30) + 8;
        bounds.y = 0;
        this.active = true;
        this.found = false;
    ...
    private void setRandomDelta() {
        randomDelta = (int) (Math.random() * 25) + 9;
    }
}
